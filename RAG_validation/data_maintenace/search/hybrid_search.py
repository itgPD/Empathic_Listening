...
def hybrid_search(search_text: str, df: pd.DataFrame) -> list[str]:
    """
    Description: テキストのラベルに対してハイブリッド検索を行う関数
    Input:
        search_text: 検索したい文章
        df: ベクトルDBに格納されているdf
    Output:
    ...
    # リランク処理
    copy_df["hybrid_score"] = (
        1 / (copy_df["vector_rank"] + 60)
        + 1 / (copy_df["keyword_rank"] + 60)
        + 1 / (copy_df["reverse_vector_rank"] + 60)
    )
